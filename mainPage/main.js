...
function createCharacter(charaType = null) {
  ...
  if (charaType) {
    ...
    newUnit["image"] =
      ...
      charaType +
      "/" +
      String(Math.floor(Math.random() * (listCrewImages[charaType] - 1))) +
      ".png";
...
function concludeBattle(victory) {
    ...
    for (let eachRow in groupOfEnemies) {
      for (let eachEnemy in groupOfEnemies[eachRow]) {
        for (let eachLoot in lootTable[
          groupOfEnemies[eachRow][eachEnemy]["charaType"]
        ]["items"]) {
          let randLuck = Math.random();
          if (
            randLuck <
            lootTable[groupOfEnemies[eachRow][eachEnemy]["charaType"]]["items"][
              eachLoot
            ]
          ) {
  ...
  finishBattleButton.setAttribute("id", "finishBattleButton");
  finishBattleButton.addEventListener("click", function () {
    document.getElementById("lootDiv").remove();
    changeMenu("turnX");
  });
  actionMenu.appendChild(finishBattleButton);
  for (let i = 1; i < 4; i++) {
...
function manageArmy() {
  ...
  numberOfEnnemy =
    ...
    Math.round(
      crewTotal.length *
        playerLocation["enemyType"][enemyType]["playerCaravanSizeFactor"] *
        Math.random()
    );
  ...
  ennemyApprochingText.textContent = `Ennemy in sight: a group of '${enemyType}s' is approaching you.\nThey are ${numberOfEnnemy}.`;
  ...
  if (playerLocation["enemyType"][enemyType]["bribable"]) {
    ...
    bribeArmy.addEventListener("click", function () {
      ...
      ennemyApprochingText.textContent =
        "You did not have enough money to bribe the enemies";
      node.appendChild(ennemyApprochingText);
                        ...
                        <div class="modal-content">
                            ...
                            <div class="modal-body">
                                <div class="modal-text">
                                    ...
                                    ${
                                      item.type === "food"
                                        ? `Type: ${item.type}<br>Restoration: ${item.foodValue}<br>Quantity: ${quantity}<br>Value: ${item.price}<br>Weight: ${item.weight}<br>Description: ${item.description}`
                                        : ""
                                    }
                                    ${
                                      item.type === "mount" ||
                                      item.type === "carrier"
                                        ? `Type: ${item.type}<br>Volume: ${item.volume}<br>Value: ${item.price}<br>Weight: ${item.weight}<br>Description: ${item.description}`
                                        : ""
                                    }
                                    ${
                                      item.type !== "weapon" &&
                                      ...
                                      item.type !== "mount" &&
                                      item.type !== "carrier"
                                        ? `Type: ${item.type}<br>Quantity: ${quantity}<br>Value: ${item.value}<br>Carry Value: ${item.carryValue}<br>Weight: ${item.weight}<br>Description: ${item.description}`
                                        : ""
                                    }
                                </div>
...
function itsHoliday() {
  const message = "It's a holiday! Everything is 20% off!";
...
function findOasis() {
  console.log("oasis found");
  water += 50;
  const message = "Your crew has found an Oasis and replenished 50 water!";
  const background = "../Images/EventBackgrounds/oasis.png";
  updateRessourcesDisplay();
  displayMessage(message);
  setCenterImage(background);
}
