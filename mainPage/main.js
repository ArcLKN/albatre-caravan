...
var locations = [
  {
    ...
    isTradable: false,
    doEnemySpawn: true,
    enemyType: { legionary: 0.7, bandit: 1 },
    description:
...
function computeFood() {
  console.log("INV FOOD", inventory);
  let foodNbr = 0;
...
function initBattle() {
  ...
  if (enemyType == "bandit") {
    numberOfEnnemy = Math.getRandomIntInclusive(
      5,
      Math.max(7, Math.round(crewTotal.length / 4))
    );
  }
...
function manageArmy() {
  ...
  ennemyApprochingText.textContent =
    "Ennemy in sight: a group of bandits is approaching you.";
