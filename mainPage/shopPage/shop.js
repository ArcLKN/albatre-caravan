...
function displayInventory() {
  ["player", "shop"].forEach((e) => {
    let eInventaire;
    if (e == "player") {
      if (!playerLocation["sellableGoods"]) {
        return;
      }
...
function sellItem(thisItem, newValue) {
  ...
  resultatNode.textContent =
    ...
    (newValue - previousValues[thisItem]) *
      playerLocation["sellableGoods"][thisItem]["price"];
  changeTriangle();
  ...
  displayQ.innerText =
    "You're selling  " + newValue + "  " + allItems[thisItem]["name"];
}
function buyItem(thisItem, newValue) {
  ...
  displayQ.innerText =
    "You're buying  " + newValue + "  " + allItems[thisItem]["name"];
}
...
function changeTriangle() {
  ...
  if (res === 0) {
    ...
    triangle.style.borderColor = "transparent white transparent transparent";
...
function update() {
  ...
  ["player", "shop"].forEach((e) => {
    ...
    for (let eachItem in eInventaire) {
      ...
      if (e == "player") {
        document.getElementById(eachItem + "Qtt").innerText =
          "Quantité dans l'inventaire : " + nvQuantity;
        document.getElementById(inputID).max = Math.min(
          nvQuantity,
          playerLocation["sellableGoods"][eachItem]["volume"]
        );
      } else {
        if (inventaire[eachItem]) {
          inventaire[eachItem]["volume"] += quantitySold;
        } else {
          inventaire[eachItem] = { volume: quantitySold };
        }
        document.getElementById(eachItem + "ShopQtt").innerText =
          "Quantité dans l'inventaire : " + nvQuantity;
...
function displayNew() {
  //y a update argent dedans aussi
  let resultatNode = document.getElementById("finali");
  let res = parseInt(resultatNode.textContent);
  let recap = document.getElementById("recap");
  recap.style.display = "inline-block";
